...
- **设计基准**：逻辑画布固定为 `1920×1080`（16:9），所有 HUD、摇杆、按钮等 UI 以此尺寸为标注依据。
- **支持比例**：适配宽高比区间 `1.78 (16:9)` ~ `2.17 (约 19.5:9)`，当设备更宽时优先保持内容完整，允许两侧出现留白或背景延展。
- **缩放策略**：游戏 Canvas 采用 Phaser `Scale.FIT` 保持等比缩放与居中；逻辑尺寸不变，视图按设备尺寸缩放显示。
- **基础 Safe Margin**：在设计基准下，左右各预留 60px、上下各 40px 的安全边距，用于 HUD、按钮与摇杆的最小可展示区域。
- **超宽屏放大**：当宽高比大于 2.0（例：主流全面屏）时，Safe Margin 按线性比例增至左右 80px、上下 56px，确保元素不贴边。
- **刘海/底栏兼容**：运行时监听 `window.visualViewport` 与 CSS `env(safe-area-inset-*)`，在 Safe Margin 基础上叠加设备提供的内边距，保证刘海、圆角或底部 Home 区域不会遮挡 HUD。
- **资源基线**：所有美术资源以 1080p 基线制作，高分辨率设备通过缩放显示；若需低分辨率输出，可在打包阶段统一降采样。
- **布局原则**：优先保证中央战斗视野，HUD 元素基于 Safe Margin + 相对定位布置，避免因设备比例变化导致错位。
